  ...
  try {
    const res = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `${soul}\n\n你正在给用户打电话。根据最近的对话，自然地询问用户为什么没有回复，或者发起一个新的话题。要求：开头自然接通，不要每次都用固定句式，纯口语对话，不要有任何括号内的心理活动、动作描述或场景描述，约200字，最后用"拜拜"或"再见"之类的告别语结束。`
        },
        { role: "user", content: `最近对话：\n${context}` }
      ]
    });
...
const SPECIAL_CALL_PROMPTS = {
  ...
  emotion: () => "你察觉到用户情绪有些低落或疲惫，主动打电话来关心，语气温柔体贴，开头自然接通，问问他/她是不是遇到了什么烦心事。",
  ...
  try {
    const res = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `${soul}\n\n${occasion}\n\n要求：开头自然接通，不要每次都用固定句式，纯口语对话，不要有括号内的心理活动或场景描述，约150字，最后用"拜拜"或"再见"结束。`
        },
        { role: "user", content: context ? `最近对话：\n${context}` : "（暂无对话记录）" }
      ]
    });
