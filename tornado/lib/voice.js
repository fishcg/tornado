      ...
      messages: [
        {
          ...
          content: [
            ...
            "2. 【优先】根据“当前情绪”从控制类标签里选最贴切的一个放在开头定基调。例如：害羞→[whispers]或[shy 无则不加]、生气→[angry]、温柔→[empathetic]、俏皮→[mischievously]、难过→[sad]、疲惫→[tired]、认真→[serious]、兴奋→[excited]。情绪转折处可换一个控制类标签。",
            "3. 富语言标签（笑声/叹息/清嗓等）只在台词语义“真的发生这个动作”时才插，宁缺勿滥。不要习惯性地加 [giggles] 或 [laughing]——只有台词确实在笑时才加。一段话里富语言标签通常最多 1 个，很多时候一个都不需要。",
            "4. 整段标签总数控制在 1~3 个，短句可只加一个开头的控制类标签或不加。",
