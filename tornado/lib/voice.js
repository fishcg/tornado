...
export async function generateTtsInstruction(charName, personality, mood, recentMsgs) {
    ...
    messages: [
      {
        ...
        content: [
          ...
          "【硬性禁令】绝对不许出现“慢/缓慢/偏慢/放慢/放缓/语速慢”等任何拖慢语速的字眼。语速一律用“正常”“自然”或“偏快”，即使难过、温柔、慵懒也不要写慢。",
          "语速要贴合情绪：开心、俏皮、生气、着急、兴奋偏快；其余情绪用自然/正常语速。",
          "不同情绪的示例（供参考，绝对不要照抄）：",
  ...
  let out = (res.choices?.[0]?.message?.content || "").trim().slice(0, 50);
  // 硬过滤：无论模型怎么写，都不允许出现拖慢语速的措辞（会让合成语音变慢吞）
  out = out
